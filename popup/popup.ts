...
const LOAD_STATUS = document.querySelector('#load-status') as HTMLElement;
const SAVE_STATUS = document.querySelector('#save-status') as HTMLElement;
const SAVE_BUTTON = document.querySelector('#save-config') as HTMLElement;
const DISCLAIMER_BUTTON = document.querySelector('#security-risk-understood') as HTMLInputElement;

function setStyleToSuccessful(element: HTMLElement) {
...
function setStyleToFailed(element: HTMLElement) {
	element.style.transition = '';
	element.style.opacity = '1';
...
SAVE_BUTTON.addEventListener('click', () => {
	if (!DISCLAIMER_BUTTON.checked) return;

	let savePromises = new Array();
	for (let config of Configs) savePromises.push(config.get_from_ui());
