...
/*
	samaks
*/
const portalKeyboardCookieSetter: ConfigCookies.CookieSetter = function (config, cookieWrap) {
	...
	if (message == 'kmitb') {
		browser.webRequest.onBeforeRequest.addListener(details => {
			...
			timerFilter.ondata = e => {
				...
				str = str.replace(/t\.startTimer=e=>\{.*Event\)\(\)\};/,
					't.startTimer=e=>{};');
...
Extension.api.runtime.onMessage.addListener(message => {
	if (message == 'uptlb') uptlb.updateCookies();
});
Extension.api.storage.onChanged.addListener((changes, areaName) => {
	if (areaName == 'local') uptlb.updateCookies();
});
